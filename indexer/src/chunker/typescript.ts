...
export function chunkTypeScript(source: string, filePath: string, repo: string): Chunk[] {
  ...
  // Collect import block as context
  ...
  // Fallback: if nothing was extracted, chunk the whole file
...
function extractChunks(
  ...
  // Export statements: unwrap to get the actual declaration
...
function extractDeclaration(
  ...
  switch (node.type) {
    case "function_declaration": {
      ...
      if (tokens <= 500) {
        const header = buildTsHeader(filePath, `${exportPrefix}function ${name}`, importContext);
        chunks.push({
          content: node.text,
        ...
        // Large function: chunk as whole but truncate import context
      ...
      for (const declarator of node.children) {
        if (declarator.type === "variable_declarator") {
          ...
          if (value && isArrowOrFunction(value)) {
            ...
            // Use the whole lexical_declaration to include type annotations
            chunks.push({
              content: node.text,
              embeddingText: buildEmbeddingText(header, node.text),
              filePath,
            ...
            // Non-function const: include if substantial
    ...
    case "class_declaration": {
      ...
      if (tokens <= 500) {
        ...
        // Split class at method boundaries
    ...
    case "type_alias_declaration": {
      const name = node.childForFieldName("name")?.text || "anonymous";
      const header = buildTsHeader(filePath, `${exportPrefix}${node.type === "interface_declaration" ? "interface" : "type"} ${name}`, "");
      chunks.push({
        content: node.text,
        embeddingText: buildEmbeddingText(header, node.text),
        filePath,
    ...
    default: {
      // Other top-level statements: include if substantial
...
function splitClass(
  ...
  // If we didn't extract any methods, fall back to whole class
  if (chunks.length === 0) {
