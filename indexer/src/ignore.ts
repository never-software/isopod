...
export function createIgnoreFilter(repoPath: string): Ignore {
  ...
  // Load .isopodignore (project-specific index exclusions)
  const isopodIgnorePath = resolve(repoPath, ".isopodignore");
  if (existsSync(isopodIgnorePath)) {
    ig.add(readFileSync(isopodIgnorePath, "utf-8"));
  }
