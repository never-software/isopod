...
function apiPodExists(res: ServerResponse, name: string): void {
  ...
  const exists = existsSync(podPath);
  json(res, 200, { exists });
}
...
function apiPodAction(res: ServerResponse, podName: string, action: "up" | "down"): void {
  ...
  execFile(isopodBin, [action, podName], { timeout: 120000 }, (error, stdout, stderr) => {
...
function getContainerStatuses(): Map<string, ContainerStatus> {
  ...
  try {
    ...
    for (const line of output.split("\n")) {
      ...
      // Container names are like "isopod-{podname}-{service}-{replica}"
      // e.g. "isopod-my-feature-workspace-1" → pod name is "my-feature"
      const match = name.match(/^isopod-(.+)-\w+-\d+$/);
      if (match) {
