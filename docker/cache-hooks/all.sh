...
# hook script. Each hook checks if it's relevant (e.g., node.sh skips repos
# without a package.json) and outputs Dockerfile instructions if so.
#
# The combined output replaces __CACHE_HOOK_INSTRUCTIONS__ in the Dockerfile.
#
...
#   REPO_FILTER      — (optional) Only run against this repo (e.g., "api")
