...
import type { Chunk, SearchResult, SearchOptions, CollectionBranchInfo } from "../types.js";
...
export async function getExistingFileHash(
  collectionName: string,
  filePath: string,
  branch: string
): Promise<string | null> {
  try {
    ...
    const hash = (results.points[0]?.payload as any)?.file_hash as string;
    return hash || null;
  } catch {
    return null;
  }
