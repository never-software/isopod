 ...
 */
export function teardownWorkspace(featureName: string, opts?: { removing?: boolean }): void {
 ...
 */
export async function waitForUrls(featureName: string, timeout = 600): Promise<UrlInfo[]> {
  const hook = join(config.dockerDir, "hooks", "urls");
  if (!existsSync(hook)) return [];
