...
**Pod templates** (`stacks/<stack>/workspace/`): files here are a one-way template source for pods in that stack. isopod copies missing files into new pod roots, and later `isopod up` copies only missing files into pods created with the template marker. Pod-local edits are never overwritten, old unmarked pods are not backfilled, and each pod root is mounted at `/workspace` with repo mounts overlaid at `/workspace/<repo>`.
